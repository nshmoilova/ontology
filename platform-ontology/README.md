...
ontology/          TBox modules (OWL 2 QL target), one file per domain
  ...
  contract.ttl       published contracts of capabilities and their versions
                     (sibling module; nothing imports it)
...
Module import chain: `control-plane → authz → authn → core`, plus the siblings `commitment → control-plane, core` and `contract → control-plane, core`, with
